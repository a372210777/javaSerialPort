...
public class SerialConnection implements SerialPortEventListener,
	...
	private sendDataThread currentThread;
	 ...
	 * to reliquish the port if have it open and no longer need it. 打开串口
	 */
	...
	public void writeData(int[] data, int type) {
		//线程首次被初始化 即开机
		if(currentThread==null){
			currentThread = new sendDataThread(data, os, type);
			currentThread.start();
		}else{
			currentThread.changeModel(false);
			currentThread = new sendDataThread(data, os, type); 
			currentThread.start();
		}
	...
	class sendDataThread extends Thread {
		...
		private boolean isAllowRun=false;
		
		public sendDataThread(int[] data, OutputStream out) {
			this.data = data;
			this.out = out;
		}

		public sendDataThread(int[] data, OutputStream out, int type) {
			this.data = data;
			this.out = out;
			this.type = type;
		...
		public void changeModel(boolean isAllow){
			
			this.isAllowRun=isAllow;
		}
	}
}
